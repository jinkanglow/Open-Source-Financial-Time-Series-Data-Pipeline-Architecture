-- TimescaleDB Schema for Financial Time-Series Data Pipeline
-- Version: 2.1
-- Last Updated: Jan 23, 2026

-- Enable TimescaleDB extension
CREATE EXTENSION IF NOT EXISTS timescaledb CASCADE;
CREATE EXTENSION IF NOT EXISTS vector CASCADE; -- For news_sentiment_embedding

-- ============================================================================
-- Raw Data Tables
-- ============================================================================

-- Market data raw table (hypertable)
...
-- Add refresh policy for ohlc_1m
...
CREATE MATERIALIZED VIEW IF NOT EXISTS sma_20_agg
WITH (timescaledb.continuous) AS
SELECT
    time_bucket('1 minute', time) AS minute,
    symbol,
...
-- Note: SMA requires a different approach - using window function
-- For continuous aggregate, we'll use a materialized view with refresh
...
-- 3. ewm_12: 12-period Exponential Moving Average
-- Using stored procedure approach for EWM calculation
...
-- 4. volatility_1h: 1-hour volatility (stddev of log returns)
...
-- 5. vwap_5m: 5-minute Volume Weighted Average Price
...
-- 6. large_trade_flag: Flag for large trades (volume > Q95)
-- This will be populated by Flink CEP, but we create the table structure
...
-- 7. bidask_spread: Bid-Ask Spread
...
-- 8. trade_imbalance_5m: 5-minute Trade Imbalance
...
-- 9. regime_tag: Market Regime Tag (up/down based on price vs SMA)
...
-- 10. news_sentiment_embedding: News Sentiment Vector Embedding
...
CREATE INDEX IF NOT EXISTS idx_news_sentiment_embedding 
    ON news_sentiment USING ivfflat (news_sentiment_embedding vector_cosine_ops);

-- ============================================================================
-- Point-in-Time Feature Snapshot Function
-- ============================================================================
...
-- Create policy function (example - customize based on your tenant model)
...
BEGIN
    ...
    RETURN QUERY SELECT current_setting('app.current_tenant', TRUE)::VARCHAR;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Example RLS policy (customize based on your needs)
-- CREATE POLICY tenant_isolation ON market_data_raw
--     FOR ALL
...
CREATE INDEX IF NOT EXISTS idx_ohlc_symbol_minute 
    ON ohlc_1m_agg (symbol, minute DESC);
CREATE INDEX IF NOT EXISTS idx_vwap_symbol_fivemin 
    ON vwap_5m_agg (symbol, five_min DESC);
CREATE INDEX IF NOT EXISTS idx_imbalance_symbol_fivemin 
    ON trade_imbalance_5m_agg (symbol, five_min DESC);
